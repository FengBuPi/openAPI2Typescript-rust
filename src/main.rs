...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let openapi_spec: OpenAPI = serde_json::from_str(&openapi_content)?;
    println!("当前openapi版本: {:?}", openapi_spec.openapi);

    // OpenAPI to TemplateData.list
    let template_data_list = utils::openapi_to_template_data_list(&openapi_spec)?;
